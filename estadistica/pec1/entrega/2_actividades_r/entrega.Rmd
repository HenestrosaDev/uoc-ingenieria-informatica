...
```{r}
# 1) Ordenar en orden decreciente y mostrar solo las 3 primeras filas
ord <- order(datos$IMDb_Rating, decreasing = TRUE)
top3 <- datos[ord, ][1:3, ]
...
```{r}
# 1) Resumen numérico
s <- summary(datos$IMDb_Rating)  # mínimo, Q1, mediana, media, Q3 y máximo

# 2) Tabla con todas las medidas
table <- as.data.frame(t(c(
  ...
  Q3      = s["3rd Qu."],
  Max     = s["Max."],
  Var     = var(datos$IMDb_Rating),
  sd      = sd(datos$IMDb_Rating)
)), check.names = FALSE)
...
```{r}
# 1) Filtrar por `Age_Rating == "R"`
x <- with(datos, IMDb_Rating[Age_Rating == "R"])
...
```{r}
# 1) Valor mínimo de la variable `IMDb_Rating`
min_val <- min(datos$IMDb_Rating)

# 2) Añadir las variables `Name` y `Category` donde se da el valor mínimo
result <- with(
  datos, 
  datos[which(IMDb_Rating == min_val), c("Name", "Category", "IMDb_Rating")]
)
...
```{r}
# 1) Mostar tabla con frecuencias absolutas
tab_abs <- table(datos$Age_Rating)
tab_abs

# 2) Mostar barplot con frecuencias absolutas
barplot(tab_abs, 
...
# 4) Mostrar tabla con porcentajes
tab_pct_with_pct

# 5) Mostrar barplot con porcentajes
barplot(tab_pct, 
...
df <- datos[, c("Duration", "Age_Rating")]

# 2) Asegurar tipo de dato correcto
df$Age_Rating <- as.factor(df$Age_Rating)
df$Duration   <- as.numeric(df$Duration)

# 3) Estratificación con el número de individuos por cada muestra
