...
import edu.uoc.epcsd.course.domain.exception.EnrollmentNotFoundException;
...
public class EnrollmentRepositoryImpl implements EnrollmentRepository {
	...
	@Override
	public Optional<Enrollment> findEnrollmentByStudent(String userEmail) {

		return jpaRepository.findEnrollmentByStudent(userEmail).map(EnrollmentEntity::toDomain);
	}
	...
	public Long createEnrollment(Enrollment enrollment) {
		...
		enrollmentEntity.setCourse(jpaCourseRepository.findById(enrollment.getCourseId()).orElseThrow(() -> new CourseNotFoundException(enrollment.getCourseId())));
	...
	public Long updateEnrollment(Enrollment enrollment) {
		...
		EnrollmentEntity enrollmentEntity = jpaRepository.findById(enrollment.getId()).orElseThrow(() -> new EnrollmentNotFoundException(enrollment.getId()));
